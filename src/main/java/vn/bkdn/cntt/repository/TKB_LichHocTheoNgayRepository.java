...
public interface TKB_LichHocTheoNgayRepository extends JpaRepository<TKB_LichHocTheoNgay, Integer> {
    ...
    @Modifying
    @Transactional
    @Query(value = "update tkb_lich_hoc_theo_ngay set ngay = ?1, tkb_thu_id = ?2, dm_giang_duong_id = ?3, " +
    ...
    @Query(value = "select * from tkb_lich_hoc_theo_ngay tkb where tkb.dm_giang_duong_id = ?1 and tkb.ngay = ?2", nativeQuery = true)
    ...
    @Query(value = "select dm_lop_mon_hoc_id from tkb_lich_hoc_theo_ngay where id = ?1", nativeQuery = true)
